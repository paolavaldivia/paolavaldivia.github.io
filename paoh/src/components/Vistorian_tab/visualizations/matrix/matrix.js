...
var MatrixLabels = (function () {
    ...
    MatrixLabels.prototype.updateData = function (leftNodes, topNodes, cellSize, nodeOrder, leftLabelOffset, topLabelOffset, bbox) {
        ...
        var labelsLeft = this.svg.selectAll('.labelsLeft')
            .data(leftNodes);
        var leftLabelPosition = function (nodeId) {
            return _this.margin.top + leftLabelOffset + cellSize * (nodeOrder[nodeId] - bbox.y0) + cellSize / 2;
        };
        ...
        var labelsTop = this.svg.selectAll('.labelsTop')
            .data(topNodes);
        var topLabelPosition = function (nodeId) {
            return _this.margin.left + topLabelOffset + cellSize * (nodeOrder[nodeId] - bbox.x0) + cellSize / 2;
        };
        labelsTop.enter().append('text')
            ...
            .attr('transform', function (d, i) {
                return 'rotate(-90, ' + (_this.margin.top + cellSize * i + cellSize / 2) + ', ' + (_this.margin.left - 10) + ')';
            })
            .on('mouseover', function (d, i) {
        ...
        labelsTop
            ...
            .attr('transform', function (d, i) {
                return 'rotate(-90, ' + (_this.margin.top + topLabelOffset + cellSize * (nodeOrder[d.id()] - bbox.x0) + cellSize / 2) + ', ' + (_this.margin.left - 10) + ')';
            });
        this.updateHighlightedNodes();
    };
    MatrixLabels.prototype.updateHighlightedNodes = function (highlightedLinks) {
        if (highlightedLinks === void 0) {
            highlightedLinks = [];
        }
        var color;
...
var MatrixVisualization = (function () {
    ...
    MatrixVisualization.prototype.updateHighlightedLinks = function (hoveredLinks) {
        if (hoveredLinks.length) {
        }
        if (hoveredLinks === void 0) {
            hoveredLinks = undefined;
        }
        ...
        for (var _d = 0, _e = this.hoveredLinks; _d < _e.length; _d++) {
            ...
            if (this.cellHighlightFrames[id])
                for (var _f = 0, _g = this.cellHighlightFrames[id]; _f < _g.length; _f++) {
                    var frame = _g[_f];
                    frame.children[0].material.color.b = 12
                    frame.children[0].material.color.r = 190
                    frame.children[0].material.color.g = 122
                    frame.children[0].visible = false;
...
var Matrix = (function () {
        ...
        for (var i_11 = 0; i_11 < leftNodes.length; i_11++) {
            ...
            if (node.isVisible()) {
                ...
                for (var _i = 0, _a = node.links().toArray(); _i < _a.length; _i++) {
                    ...
                    if (!link.source) {
                        console.trace();
                        console.error('No link source in matrix');
                    }
                    var neighbor = link.source.id() == node.id() ? link.target : link.source;
                    if (neighbor.isVisible() &&
    ...
    Matrix.prototype.highlightLinks = function (highlightedIds) {
        ...
        if (highlightedIds.length > 0) {
            var highlightedLinks = highlightedIds.map(function (d) {
                return _this._dgraph.link(d);
            });
            networkcube.highlight('set', {
    ...
    Matrix.prototype.nodeClicked = function (d) {
        ...
        for (var j = 0; j < selections.length; j++) {
            if (selections[j] == currentSelection) {
                networkcube.selection('remove', {
                    nodes: [d]
                });
                return;
            }
        }
        networkcube.selection('add', {
            nodes: [d]
        });
    ...
    Matrix.prototype.highlightNodes = function (highlightedIds) {
        var _this = this;
        if (highlightedIds.length > 0) {
            var highlightedNodes = highlightedIds.map(function (d) {
                return _this._dgraph.node(d);
            });
            networkcube.highlight('set', {
...
var appendToBody = function (domId) {
