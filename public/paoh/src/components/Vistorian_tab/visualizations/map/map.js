...
function init() {
    ...
    overlay.onAdd = function () {
        ...
        vNodeLabelBackgrounds = visualNodes.append("rect")
            ...
            .attr("width", function (d) {
                return getTextWidth(createNodeLabel(d));
            })
            .attr("height", 18)
            .attr('visibility', 'hidden');
        vNodeLabels = visualNodes.append('text')
            .text(function (d) {
                return createNodeLabel(d);
            })
    ...
    map.addListener('mousemove', function (ev) {
        ...
        for (var i = 0; i < nodePositionObjects.length; i++) {
            pos = projection.fromDivPixelToLatLng({
                x: nodePositionObjects[i].x,
                y: nodePositionObjects[i].y
            });
            pos = {
                x: pos.lng() * F,
                y: pos.lat() * F
        ...
        if (intersectedNode == undefined) {
            ...
            for (var i = 0; i < _this.links.length; i++) {
                ...
                if (sourceNPO == undefined) {
                    ...
                    sourcePoint = projection.fromDivPixelToLatLng({
                        x: sourceNPO.x,
                        y: sourceNPO.y
                    });
                    sourcePoint = {
                ...
                if (targetNPO == undefined) {
                    ...
                    targetPoint = projection.fromDivPixelToLatLng({
                        x: targetNPO.x,
                        y: targetNPO.y
                    });
                    targetPoint = {
...
function updateLinks() {
    if (!visualLinks) {
        console.error('No visuaLinks in Map')
        console.trace();
    } else {
        visualLinks
...
function updateNodes() {
    ...
    if (!visualNodes) {
        console.trace();
        console.error('no visualNodes in map ')
    } else {
        visualNodes
            .attr('opacity', function (d) {
                if (!d.node.isVisible())
                    ...
                    for (var i = 0; i < visibleLabels.length; i++) {
                        ...
                        if (npo1.node == npo2.node) {
                            ...
                            vNodeLabelBackgrounds
                                ...
                                .attr('x', function (d) {
                                    return -LABEL_OFFSET_X - getTextWidth(createNodeLabel(d));
                                })
                                .attr('text-anchor', 'end')
                                .attr('visibility', 'visible');
                            visible = true;
        ...
        vNodeLabelBackgrounds
            .filter(function (m) {
                return visibleLabels.indexOf(m) > -1;
            })
            .attr('visibility', 'visible');
        vNodeLabelBackgrounds
            .filter(function (m) {
